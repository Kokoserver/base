...
from base.baseMixin.accountMixin import UserMixin
from base.baseMixin.generalMixin import base
from base.model.userModel.accountModel import User
from base.model.userModel.userPydanticModel import (UserInput, UpdateUserInput,
                                                        GetPasswordResetLink,
                                                        PasswordResetInput, UserLoginInput)
from base.settings import WEBSITE_NAME
...
async def index(user: UserInput, background: BackgroundTasks):
    if User.find_user_with_email(email=user.email):
        return base.response({"message": "Account already exist"},
    ...
    if user.password.strip() == user.confirm_password.strip():
        try:
            ...
            if newUser:
                background.add_task(
                    base.mailUsr, email_title=f"Account activation",
                ...
                return base.response({"message": "Account created successfully,"
                                                     " an verification link has been sent to your email"},
                                         status_code=status.HTTP_201_CREATED)
            return base.response({"message": "Error Creating account, please check your details or try"})
            ...
            return base.response(
                ...
                status_code=status.HTTP_400_BAD_REQUEST)
    return base.response({"message": "Password is does not match"}, status_code=status.HTTP_400_BAD_REQUEST)
        ...
        if user.active:
            if UserMixin.check_password(userIn.password, user.password):
                ...
                if encode_jwt_access and encode_jwt_refresh:
                    ...
                    return base.response(SuccessResponseData,
                                             status_code=status.HTTP_200_OK)
            ErrorResponseData = {"message": "Password does not match"}
            return base.response(ErrorResponseData,
...
async def activate_user(user: UpdateUserInput):
    ...
    if update_user:
        ...
        return base.response(
            ...
            status_code=status.HTTP_200_OK)
    return base.response({"message": "Account does not exist"},
                             status_code=status.HTTP_400_BAD_REQUEST)
...
async def get_password_reset_link(background: BackgroundTasks, user: GetPasswordResetLink):
    ...
    if current_user:
        background.add_task(base.mailUsr,
                            user_email=user.email,
        ...
        if check_user:
            ...
            if password:
                ...
                return base.response(
                    ...
                    status_code=status.HTTP_200_OK)
            return base.response({"message": "Error updating password"},
                                     status_code=status.HTTP_400_BAD_REQUEST)
        return base.response({"message": "Account does not exist"},
                                 status_code=status.HTTP_400_BAD_REQUEST)
    return base.response({"message": "Password does not match"},
                             status_code=status.HTTP_400_BAD_REQUEST)
